...
function findTarget(attacker: Tank, enemies: Tank[]): Tank | null {
  ...
  const inRange = alive.filter(
    (e) => Math.abs(e.col - attacker.col) <= attacker.range,
  );
...
function buildEnemyTanks(stage: number, hpMult: number): Tank[] {
  ...
  return STAGES[idx].map((f) =>
    createTank(f.type, f.level, f.col, f.row, "enemy", hpMult),
  );
}
  ...
  switch (action.type) {
    ...
    case "PLACE_TANK": {
      ...
      const occupied = state.playerTanks.some(
        (t) => t.col === action.col && t.row === action.row && t.isAlive,
      );
    ...
    case "MERGE_TANKS": {
      ...
      return {
        ...
        selectedTankId: null,
        playerTanks: [
          ...state.playerTanks.filter(
            (t) => t.id !== action.sourceId && t.id !== action.targetId,
          ),
          merged,
        ],
      };
    ...
    case "END_BATTLE": {
      const stagesWon =
        action.result === "win" ? state.stagesWon + 1 : state.stagesWon;
...
export function useMergeTankGame(initialDifficulty: Difficulty = "medium") {
  ...
  const buyTank = useCallback(
    (tankType: TankType) => dispatch({ type: "BUY_TANK", tankType }),
    [],
  );
  ...
  const placeTank = useCallback(
    (col: number, row: number) => dispatch({ type: "PLACE_TANK", col, row }),
    [],
  );
  const selectTank = useCallback(
    (id: string | null) => dispatch({ type: "SELECT_TANK", id }),
    [],
  );
  const moveTank = useCallback(
    (id: string, col: number, row: number) =>
      dispatch({ type: "MOVE_TANK", id, col, row }),
    [],
  );
  const mergeTanks = useCallback(
    (sourceId: string, targetId: string) =>
      dispatch({ type: "MERGE_TANKS", sourceId, targetId }),
    [],
  );
  const startBattle = useCallback(() => dispatch({ type: "START_BATTLE" }), []);
  const endBattle = useCallback(
    (result: "win" | "lose") => dispatch({ type: "END_BATTLE", result }),
    [],
  );
  const nextStage = useCallback(() => dispatch({ type: "NEXT_STAGE" }), []);
  const resetGame = useCallback(
    (difficulty: Difficulty) => dispatch({ type: "RESET", difficulty }),
    [],
  );
  ...
  const getTanksAtRound = useCallback(
    (round: number): Tank[] => {
      const all = [
        ...state.initialBattlePlayerTanks,
        ...state.initialBattleEnemyTanks,
      ];
      if (round === 0 || !state.battleLog.length) return all;
      const snap =
        state.battleLog[Math.min(round - 1, state.battleLog.length - 1)]
          .tankStates;
      return all.map((t) => ({
        ...t,
